...
class MultiDecoder(nn.Module):
    ...
    def forward(self, x_EEG, x_EOG):
        ...
        x_EEG  = x_EEG .contiguous().view(-1, 10, c1, d1)
        x_EOG = x_EOG.contiguous().view(-1, 10, c1, d1)
        return x_EEG, x_EOG
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.classfier = CLS(256*20, 256, 2)
        self.MultiDecoder = MultiDecoder()

if __name__ == '__main__':
    device = torch.device('cuda')
    data_m1 = torch.randn((2, 10, 1, 3000)).to(device)
    data_m2 = torch.randn((2, 10, 1, 3000)).to(device)
    x11 = torch.randn((2, 10, 128)).to(device)
    x22 = torch.randn((2, 10, 128)).to(device)
    model = Model().to(device)
    out1, out2 = model.MultiEncoder(data_m1, data_m2)
    ...
    out_m1_to_m2 = model.imputation1(out1)
    out_m2_to_m1 = model.imputation2(out2)
    ...
    out7, out8 = model.MultiDecoder(out_MKLD11, out_MKLD22)
